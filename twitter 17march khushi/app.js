...
app.post('/like', async(req, res) => {
   ...
   var f =`select like_flag from tweets where id='${pid}'`;
   var data = await getdata(f);
       console.log(data[0].like_flag)
    if(data[0].like_flag==0){
        console.log("data is 0")
        var sql=`update tweets set likes = likes + 1 ,like_flag=1 where id = '${pid}' and user_id='${uid}'`;
        var result = await getdata(sql);
        console.log("liked")
         res.redirect("/home");
    }
    if(data[0].like_flag==1){
        console.log("data is 1")
        var sql=`update tweets set likes = likes - 1 ,like_flag=0 where id = '${pid}' and user_id='${uid}'`;
        var result = await getdata(sql);
